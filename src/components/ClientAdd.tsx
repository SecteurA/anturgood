...
const ClientAdd: React.FC<ClientAddProps> = ({ onNavigateBack }) => {
  ...
  // Generate next client number from database
  ...
  // Load next client number on component mount
  React.useEffect(() => {
    generateClientNumber().then(setNextClientNumber);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert([
          {
            numero_client: nextClientNumber,
      ...
      if (error.code === '23505') {
        if (error.details?.includes('numero_client')) {
          setErrors({ general: 'Ce numéro client existe déjà' });
